...
struct SHA256 {
    ...
    static func hexStringDigest (input : String) -> NSData {
        let data = SHA256.dataFromHexString(input)
        return digest(data)
    }
    
    // Returns a string representation of a hexadecimal number
    static func hexStringDigest (input : NSData) -> String {
    ...
    static func hexStringDigest (input : String) -> String {
        let digest: NSData = hexStringDigest(input)
        return hexStringFromData(digest)
    }
    
    static func dataFromHexString(input: String) -> NSData {
